...
class Apriori(minSupport: Double, minConfidence: Double, data: List[String]) {
  ...
  val transactions = data.map(transaction => (transaction.split(",").toSet))
  ...
  def generateFrequentItemSet: List[(Set[String], Double)] = {
    val items = transactions.reduce((x1, x2)=>x1++x2)
    val lastFrequentSet = items.map(item=>(item, countSupport(string2Set(item)))).filter(_._2>=minSupport).toList
